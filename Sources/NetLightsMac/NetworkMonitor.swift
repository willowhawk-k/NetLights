...
final class NetworkMonitor: ObservableObject, TopologyCollector {
    ...
    func refresh() {
        ...
        var newEgress = computeEgress(routes: newRoutes, interfaces: newInterfaces, rank: rankNow)
    ...
    // computeEgress + buildGatewayNodes moved to NetLightsCore/Normalize.swift (pure,
    // portable transforms over the model). currentSSID stays here (CoreWLAN, macOS).
